...
def keras_model():
    ...
    model.add(layers.Dense(300, input_dim=col_number, activation='relu', kernel_initializer='normal', kernel_regularizer='l2'))
    model.add(layers.Dropout(0.5))
    model.add(layers.Dense(200, activation='relu'))
    model.add(layers.Dropout(0.5))
    model.add(layers.Dense(100, activation='relu'))
    model.add(layers.Dropout(0.5))
    model.add(layers.Dense(50, activation='relu'))  #,  kernel_regularizer='l2'
...
# Define model
# estimator = RandomForestRegressor(n_estimators=500, criterion='mse', verbose=10)

# estimator = KNeighborsRegressor(n_neighbors=20)

# estimator = SGDRegressor(loss='squared_loss', penalty='l2', alpha=0.0001, l1_ratio=0.15, fit_intercept=True, max_iter=1000, tol=0.001, shuffle=True, verbose=10)

estimator = svm.SVR(verbose=True)
